...
export default function AdminCameras() {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (modalMode === "add") {
                ...
                // 🔥 Nếu là edit, cập nhật thông tin camera cho tất cả user sở hữu camera này
                if (res.ok) {
                    await updateCameraForAllUsers(editingCamera.id, {
                        cameraName,
                        streamUrl,
                        location,
                        status,
    ...
    // 🔥 Cập nhật thông tin camera cho tất cả user sở hữu camera này
    const updateCameraForAllUsers = async (cameraId, updatedData) => {
        try {
            // Lấy tất cả users
            ...
            for (const userDoc of usersSnapshot.docs) {
                ...
                // Kiểm tra xem user này có camera này không
                const userCamerasRef = collection(db, "users", userId, "cameras");
                const camerasSnapshot = await getDocs(userCamerasRef);

                for (const camDoc of camerasSnapshot.docs) {
                    if (camDoc.id === cameraId || camDoc.data().refId === cameraId) {
                        // Cập nhật thông tin camera
                        await updateDoc(doc(db, "users", userId, "cameras", camDoc.id), {
                            ...updatedData,
                        });
                    }
                }
            ...
            console.error("Lỗi khi cập nhật camera cho users:", err);
        }
    };
